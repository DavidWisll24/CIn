...
#FUNÇÃO
def melhor_partida(candidatos, j_bra, j_marr):
    maior_pontuacao = -999
    nome_melhor = ''

    for jogador in candidatos:
        gols = ass = ca = cv = 0
        if 'gols' in candidatos[jogador]:
            gols = candidatos[jogador]['gols']

        if  'ass' in candidatos[jogador]:
            ass = candidatos[jogador]['ass']

        if 'amarelo' in candidatos[jogador]:
            ca = candidatos[jogador]['amarelo']

        if 'vermelho' in candidatos[jogador]:
            cv = candidatos[jogador]['vermelho']

        if ca < 2:
            pontos = (gols * 8) + (ass * 5) - (ca * 2) - (cv * 5)
        else:
...
for jogador_campo_mar in campo_marrocos:
...
if len(esquema) != 3: #*Se a formação não estiver no formato a-b-c
    ...
    if not validade_convocacao: #*Se não for válida a convocação
        ...
        while continuar:
            ...
            if not tempo_atual.isnumeric():
                continuar = False
                if not primeiro_input:
                    print("Entrada inválida. O jogo não foi iniciado!")
            
            elif int(tempo_atual) > 90 or int(tempo_atual) < 1:
                continuar = False
                if not primeiro_input:
                    print("Entrada inválida. O jogo não foi iniciado!")
            
            elif int(tempo_atual) < tempo_antigo:
                continuar = False
                if not primeiro_input:
                    print("Entrada inválida. O jogo não foi iniciado!")
                ...
                ###AÇÕES
                acoes_validas = ['gol', 'cartão amarelo', 'cartão vermelho', 'substituição']
                primeiro_input = True
                acao = str(input())
                if acao.lower() not in acoes_validas:
                    ...
                    jogador_acao = str(input())
                    tempo_antigo = int(tempo_atual)
                    ...
                    if not validade_acao:
                        if jogador_acao in jogadores_brasileiros or jogador_acao in jogadores_marroquinos:
                        ...
                        if acao.lower() =="gol":
                            if jogador_acao in convocacao_brasil:
                                gols_brasil += 1
                            ...
                            elif jogador_acao in convocacao_marrocos:
                                gols_marrocos += 1
                                if jogador_acao not in jogadores_gols:
                            ...
                            assistencia = str(input())

                            if assistencia.lower() == "sim":
                                jogador_ass = str(input())

                                if jogador_ass in convocacao_brasil:
                                ...
                                elif jogador_ass in convocacao_marrocos:
                                    if jogador_ass not in jogadores_assistencia:
                                ...
                                comentario.append(('gca', tempo_atual,  jogador_acao, jogador_ass))
                                
                            elif assistencia.lower() == 'não':
                                comentario.append(('gs', tempo_atual,  jogador_acao))
                            ...
                            if jogador_acao in convocacao_brasil:
                                if jogador_acao not in jogadores_cart_a:
                                    convocacao_brasil[jogador_acao]['amarelo'] = 1
                                    jogadores_cart_a.append(jogador_acao)

                                else:
                                    convocacao_brasil[jogador_acao]['amarelo'] += 1
                                    convocacao_brasil[jogador_acao]['vermelho'] = 1
                                    expulsos[jogador_acao] = convocacao_brasil[jogador_acao]
                                    convocacao_brasil.pop(jogador_acao)
                                    em_campo.remove(jogador_acao)


                            if jogador_acao in convocacao_marrocos:
                                if jogador_acao not in jogadores_cart_a:
                            ...
                            comentario.append(('ca', tempo_atual,  jogador_acao))
                            
                            if jogador_acao in expulsos:
                                comentario.append(('cv', tempo_atual,  jogador_acao))
                            ...
                            jogador_novo = str(input())
                            
                            if jogador_acao in em_campo:
                                em_campo.remove(jogador_acao)
                                substituidos[jogador_acao] = convocacao_brasil[jogador_acao]
                                convocacao_brasil.pop(jogador_acao)
                            
                            elif jogador_acao in campo_marrocos:
                                campo_marrocos.remove(jogador_acao)
                                substituidos[jogador_acao] = convocacao_marrocos[jogador_acao]
                                convocacao_marrocos.pop(jogador_acao)

                            if jogador_novo in convocacao_marrocos and sub_marrocos < 5:
                                campo_marrocos.append(jogador_novo)
                                convocacao_marrocos[jogador_novo]['entrou'] = True
                                sub_marrocos += 1

                            elif jogador_novo in convocacao_brasil and sub_brasil < 5:
        ...
        if primeiro_input:
            ...
            for i in convocacao_brasil:
                if convocacao_brasil[i] != {}:
                    candidatos[i] = convocacao_brasil[i]
